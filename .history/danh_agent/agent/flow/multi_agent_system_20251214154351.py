...
Architecture:
1. User Chat Agent (qwen3-coder) - Friendly interface, learns from Slurm agent outputs
2. Slurm Planning Agent (qwen3-coder) - Generates structured command sequences  
3. Execution Agent - Validates and executes commands
...
When to transfer:
- User explicitly asks to "create", "generate", "submit", or "run" commands
...
"""
    ...
    async def run_turn(self, agent: Agent, messages: List[Dict]) -> Response:
        ...
        while iteration < max_iterations:
            ...
            for tool_call in message.tool_calls:
                ...
                # Execute function
                result = tools_map[func_name](**args) if func_name in tools_map else f"Unknown tool: {func_name}"
    ...
    async def run_conversation(self, initial_message: str):
        ...
        print("  2. Slurm Planning Agent - Command generation")
        print("  3. Execution Agent - Validation & execution")
        print("\n" + "="*70 + "\n")
