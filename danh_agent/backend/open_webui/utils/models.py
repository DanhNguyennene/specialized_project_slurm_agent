...
from open_webui.routers import openai, ollama, open_agent
...
async def fetch_agent_models(request: Request, user: UserModel = None):
    agent_response = await open_agent.get_all_models(request, user=user)
...
async def get_all_base_models(request: Request, user: UserModel = None):
    ...
    agent_task = (
        fetch_agent_models(request, user)
        if request.app.state.config.ENABLE_OPENAI_API
        else asyncio.sleep(0, result=[])
    )
    ...
    openai_models, ollama_models, agent_models = await asyncio.gather(
        openai_task, ollama_task, agent_task
    )

    return agent_models + openai_models + ollama_models
